...
public interface DetalleProductoRepository extends JpaRepository<DetalleProducto, Serializable> {
	@Query(value="select dp.id as detalleId, dp.producto_id as productoId ,dp.descripcion, dp.cantidad,dp.iva, dp.precio, dp.sub_total, dp.venta_id, p.precio_venta_1, p.precio_venta_2, p.precio_venta_3, p.precio_venta_4, dp.descuento, unidad_medida.descripcion as unidad, p.existencia, dp.is_balanza,p.codbar as procodbar, m.descripcion as descrimarca, dp.monto_iva as monIva, dp.costo, dp.tipo_precio as tpPrecio, dp.cantidad_devolucion as cantDevol, dp.sub_total_costo, p.nombre_imagen as nmImg, p.codoriginal as original, dep.descripcion as despo, p.stock_presupuesto as stoPresu from detalle_producto dp inner join producto p on dp.producto_id=p.id inner join unidad_medida on p.unidad_medida_id=unidad_medida.id inner join marca m on p.marca_id=m.id inner join deposito dep on p.deposito_id=dep.id where dp.venta_id=:id",nativeQuery=true)
