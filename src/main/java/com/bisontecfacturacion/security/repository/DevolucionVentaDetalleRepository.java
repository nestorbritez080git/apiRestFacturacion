...
public interface DevolucionVentaDetalleRepository extends JpaRepository<DevolucionVentaDetalle, Serializable>{
	@Query(value = "select det.id as iddet,det.descripcion as des, ud.descripcion as ud, detprod.cantidad as cant, detprod.precio as precio, detprod.iva as iva, detprod.monto_iva as monIva, detprod.sub_total as subtotal, det.cantidad as cantidadDevol, det.sub_total as subTotalDevol, detprod.id as idDetalle, p.id as idProducto, det.precio as precioDevol   from devolucion_venta_detalle det inner join detalle_producto detprod on detprod.id=det.detalle_producto_id inner join producto p on p.id= detprod.producto_id inner join unidad_medida ud on ud.id=p.unidad_medida_id where det.devolucion_venta_id=:id", nativeQuery = true)
	...
	@Query("SELECT COALESCE(SUM(d.cantidad), 0) " +
		       ...
		       "WHERE d.detalleProducto.id = :idDetalle " +
		       "AND d.devolucionVenta.estado = 'CERRADO'")
	Double cantidadDevueltaConfirmada(@Param("idDetalle") int idDetalle);
