...
public interface CompraDetalleRepository extends JpaRepository<DetalleCompra, Serializable>{
    @Query(value = "SELECT d.id as detId, p.id as proId, p.codbar, d.descripcion,d.cantidad,d.precio_costo,d.iva,d.sub_total, d.precio_venta_1, d.precio_venta_2, d.precio_venta_3, d.precio_venta_4, d.compra_id, u.descripcion as descripcionDescrip, m.descripcion as marcadesc, dep.descripcion as deposit, p.existencia as exist, p.stock_presupuesto as soli FROM detalle_compra d INNER JOIN producto p ON d.producto_id=p.id INNER JOIN unidad_medida u ON p.unidad_medida_id=u.id INNER JOIN marca m ON p.marca_id=m.id  INNER JOIN deposito dep ON p.deposito_id=dep.id where d.compra_id=:id order by d.id desc", nativeQuery = true)
