...
public interface MovimientoE_SRepository extends JpaRepository<MovimientoEntradaSalida, Serializable> {
    ...
    @Query(value="SELECT m.descripcion, m.cantidad, m.egreso, m.ingreso, m.referencia, m.fecha, m.hora, m.marca, m.costo_salida, pf.nombre || ' ' || pf.apellido AS funcionario, tipo.descripcion as tipo, tipo.id as idTipo, costo_entrada as costoEntrada FROM movimiento_entrada_salida m inner join funcionario fun on fun.id=m.funcionario_id inner join persona pf on pf.id=fun.persona_id inner join tipo_movimiento tipo on tipo.id=m.tipo_movimiento_id where m.producto_id=:id order by m.id DESC",nativeQuery=true)
    List<Object[]>  getMovimientoProductoPorIdPRoducto(@Param("id") int id);
    
    @Query(value="SELECT SUM(cantidad * costo_entrada) / SUM(cantidad)as ccp " + 
    		"FROM movimiento_entrada_salida " + 
    		"WHERE concepto_id IN (3, 4) AND cantidad > 0 and producto_id=:id",nativeQuery=true)
    Double getCostoPromedioPonderado(@Param("id") int id);    
